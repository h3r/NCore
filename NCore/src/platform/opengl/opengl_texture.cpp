...
namespace NC {
		...
		: m_path(path)
	{
		...
		if (data)
		{
			...
			nc_assert(internalFormat & dataFormat, "Format not supported!");

			glCreateTextures(GL_TEXTURE_2D, 1, &m_render_id);
			glTextureStorage2D(m_render_id, 1, internalFormat, m_width, m_height);
	...
	void OpenGLTexture2D::SetData(void* data, uint32_t size)
	{
		...
		nc_assert(size == m_width * m_height * bpp, "Data must be entire texture!");
